...
const Cart = () => {
    ...
    useEffect(() => {
        ...
        if (userEmail) {
            const fetchCartItems = async () => {
                try {
                    const response = await axios.get(`http://localhost:6005/cart?email=${userEmail}`);
    ...
    const handleRemove = async (eventName) => {
        try {
            const response = await axios.post('http://localhost:6005/cart/remove', { eventName, userEmail });
